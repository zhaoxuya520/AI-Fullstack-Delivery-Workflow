...
content_template = """# {slug} Evolution Protocol
...
for workflow_dir in root.iterdir():
    if not workflow_dir.is_dir():
        continue
    path = workflow_dir / "EVOLUTION.md"
    if path.exists():
        continue
    path.write_text(content_template.format(slug=workflow_dir.name), encoding="utf-8")
    print(f"created {path}")

print("workflow evolution files completed")
